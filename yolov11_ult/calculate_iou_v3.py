...
def match_and_evaluate(ground_truth, detections, distance_threshold=50, iou_threshold=0.5):
    ...
    i oblicza IoU dla dopasowanych par, uwzględniając False Positives i False Negatives.
    """
    matched = []
    ...
    # Uwzględnij FP i FN jako IoU=0
    iou_scores.extend([0] * len(unmatched_detections))  # FP
    iou_scores.extend([0] * len(unmatched_ground_truth))  # FN
...
def main():
    ...
    total_mean_iou_numerator = 0  # Licznik IoU (uwzględniający FP i FN)
    total_objects = 0             # Łączna liczba obiektów (GT + Detekcje)
    ...
    for gt_file, det_file in zip(gt_files, det_files):
        ...
        total_mean_iou_numerator += results["mean_iou"] * (len(ground_truth) + len(detections))
        total_objects += len(ground_truth) + len(detections)
        total_false_positives += results["false_positives"]
        total_false_negatives += results["false_negatives"]

    # Obliczenie końcowej średniej IoU (w tym FP i FN)
    overall_mean_iou = total_mean_iou_numerator / total_objects if total_objects > 0 else 0
